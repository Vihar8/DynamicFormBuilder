...
// app/publishedform/FormPageClient.js
"use client";
...
export default function FormPageClient() {
  ...
  const formId = searchParams.get("form_id"); // or your query param

  const [responses, setResponses] = useState([]);
  const [loading, setLoading] = useState(true);

  const fetchResponses = async () => {
    const response = await formresponses({ form_id: formId });
            ...
            <tbody className="bg-white divide-y divide-gray-100">
              {responses.map((row, index) => (
                <tr key={index} className={`hover:bg-gray-50 ${index % 2 === 0 ? "bg-white" : "bg-gray-50"}`}>
                  {Object.values(row).map((val, idx) => (
                    <td key={idx} className="px-4 py-3 text-sm text-gray-800 whitespace-nowrap">
                      {val}
                    </td>
                  ))}
                </tr>
